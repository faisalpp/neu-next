const FaqTab = require('../../server/models/faqTab')
...
const faqController = {
    ...
    async createFaq(req, res, next) {
      ...
      async deleteFaq(req,res,next){
        ...
        // 1. validate user input
        const faqTabDeleteSchema = Joi.object({
            _id: Joi.string().required(),
          });
          const { error } = faqTabDeleteSchema.validate(req.body);
          ...
          try { 
            const deleteFaq = await Faq.findByIdAndDelete({_id:_id});
    
            return res.status(200).json({status:200,msg:'Faq Deleted Successfully!'});
